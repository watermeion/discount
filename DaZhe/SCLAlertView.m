...
-(void)viewWillLayoutSubviews
{
    ...
    if (self.view.superview != nil)
    {
        ...
        r = CGRectMake((sz.width-kWindowWidth)/2, (sz.height-kWindowHeight)/3, kWindowWidth, kWindowHeight);
    }
    else
    {
        // View is not visible, position outsides screen bounds
...
-(SCLAlertViewResponder *)showTitle:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        ...
        frame.origin.y = self.rootViewController.view.center.y-300;
